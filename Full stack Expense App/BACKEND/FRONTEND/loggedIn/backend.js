...
logOut.addEventListener("click", (e) => {
  e.preventDefault();
  location.replace("http://127.0.0.1:3000/logIn/login.html");
    ...
    .then((response) => {
      let options = {
        ...
        handler: function (response) {
          ...
          location.replace("http://127.0.0.1:3000/loggedIn/premiumIndex.html");
        },
      };
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (!localStorage.getItem("token")) {
    return location.replace("http://127.0.0.1:3000/logIn/login.html");
  }
  axios
  ...
  axios
    .get(
      `http://127.0.0.1:3000/expense/pagination?page=${page}&limit=${localStorage.getItem(
        "limit"
      )}`,
      {
        headers: { Authorization: localStorage.getItem("token") },
